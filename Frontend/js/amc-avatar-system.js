...
class AMCAvatar {
    ...
    loadUserData() {
        ...
        if (userString) {
            try {
                ...
                console.log('🔧 AMC Avatar: Loaded user role:', this.user.role);
            } catch (e) {
                console.error('Error parsing user data:', e);
                // Try to get role from individual storage key
                const roleFromStorage = localStorage.getItem('userRole') ||
                                      sessionStorage.getItem('userRole') ||
                                      'client_user'; // Better default than media_user
                this.user = { email: 'user@example.com', role: roleFromStorage };
            }
        } else {
            // Try to get role from individual storage key as fallback
            const roleFromStorage = localStorage.getItem('userRole') ||
                                  sessionStorage.getItem('userRole') ||
                                  'client_user'; // Better default than media_user
            this.user = { email: 'user@example.com', role: roleFromStorage };
            console.log('🔧 AMC Avatar: No user data found, using fallback role:', roleFromStorage);
        }
    }
    ...
    createAvatarHTML() {
        console.log('🔍 AMC Avatar: Looking for avatar container...');
        
        // Find the existing avatar container that's already in the header
        let avatarContainer = document.querySelector('.amc-avatar-container') ||
                             document.getElementById('avatarContainer') ||
                             document.getElementById('amcAvatar');
        
        console.log('🔍 Found container:', avatarContainer);
        
        if (!avatarContainer) {
            console.log('⚠️ No container found, creating fallback...');
            // Fallback: Create new avatar in header
            const headerRight = document.querySelector('.amc-header-right') ||
                               document.querySelector('.user-info') ||
                               document.querySelector('.app-header .user-info');
            
            console.log('🔍 Found header right:', headerRight);
            
            if (headerRight) {
                avatarContainer = document.createElement('div');
                avatarContainer.id = 'amcAvatar';
                avatarContainer.className = 'amc-avatar-container';
        ...
        if (avatarContainer) {
            console.log('✅ Injecting avatar HTML into container');
            ...
            // Dispatch event to notify header system that avatar is ready
            window.dispatchEvent(new CustomEvent('avatarSystemReady'));
            console.log('✅ Avatar system ready event dispatched');
        } else {
            console.error('❌ No avatar container found and could not create fallback!');
        }
    }
...
// Auto-initialize with small delay to ensure header is fully rendered
setTimeout(() => {
    console.log('🚀 Starting avatar system initialization...');
    initAMCAvatar();
}, 100);