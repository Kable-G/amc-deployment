/* ============================================================
   AutoMediaCenter - Enterprise Sidebar Component (REFINED)
...
class AMCSidebar {
  ...
  /**
   * Get user role from storage with multiple fallbacks
   */
  getUserRoleFromStorage() {
    try {
      ...
      if (userString) {
        ...
        if (user.role) {
          console.log('🔧 AMC Sidebar: Loaded user role from user object:', user.role);
          return user.role;
        }
      }
      ...
      if (roleFromStorage) {
        console.log('🔧 AMC Sidebar: Loaded user role from role key:', roleFromStorage);
        return roleFromStorage;
      }
      
      // Final fallback - better default
      console.log('🔧 AMC Sidebar: No role found, using client_user default');
      return 'client_user';
      
    } catch (e) {
      console.error('[AMCSidebar] Error getting user role:', e);
      return 'client_user'; // Better default than media_user
    }
  }
  ...
  generateItemHTML(item) {
    ...
    const ariaCurrent = isActive ? ' aria-current="page"' : '';  // ✅ ADD THIS LINE
...
toggleBtn.addEventListener('click', () => {
   ...
   * (Now just triggers the link tooltips system which handles hamburger)
   */
  updateTooltip() {
    const isCollapsed = document.body.classList.contains('sidebar-collapsed') ||
  ...
  /**
   * Update link tooltips based on sidebar state
   */
  ...
  /**
   * COLLAPSIBLE SIDEBAR SECTIONS - EXACT FROM test_globalshell.html
   */
  ...
  /**
   * COLLAPSED SIDEBAR: ELEGANT COLORED SECTION BARS - EXACT FROM test_globalshell.html
   */
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (sidebarContainer) {
    // Get user role from auth system with better fallbacks
    let userRole = 'client_user'; // Better default
    try {
      ...
      if (userString) {
        const user = JSON.parse(userString);
        userRole = user.role || 'client_user';
      } else {
        // Try individual role key
      ...
      console.log('🔧 AMC Sidebar Auto-Init: Using role:', userRole);
    } catch (e) {
      console.warn('[AMCSidebar] Could not parse user role, defaulting to client_user');
